...
\begin{pr}[Recovering ring of integers from valuation]\llabel{pr:val-OK}
...
Suppose $K$ is a field with a discrete nonarchimedean valuation $|\cdot|$. Let $\pi$ be a \vocab{local uniformizing parameter}, i.e. the largest element of $K$ with $|\pi|<1$ (i.e., $\pi$ is a uniformizer in the completion $K_{\pi}$). Equivalently, $\pi$ generates the maximal ideal $\mm$ in the subring of $\pi$-integers.
...
Now if $K$ is a field with a discrete nonarchimedean valuation, then we can define $A=\cO_K$ and $\mm$ as in Proposition~\ref{pr:val-OK}. The topology given by the valuation is exactly the topology that has the neighborhood base of 0 being $\{\mm^n\}_{n\ge 0}$. This shows the following.
\begin{pr}
