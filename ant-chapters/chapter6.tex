...
%See 18.705 notes.
%\end{proof}
In light of Theorem~\ref{galois}.\ref{inv-limit-comp-seq}, we can phrase this definition in a more abstract way:
\[
...
Let $K/\Q$ be an algebraic extension. Then the places on $K$ are exactly the $\mfp$-adic valuations $|\cdot|_{\mfp}$ for $\mfp$ a prime ideal of $\sO_K$. %for some prime ideal $\mfp$ of $A:=\sO_K$. 
\end{pr}
\begin{proof}
...
\end{proof}
\subsection{Completions of archimedean fields}
...
\end{pr}
%\begin{proof}
%Indeed, elements of $\sO_{\hat{K}}$ are given by $\sum_{n\ge 0}a_n\pi^n$; we can think of this as an element in $A/
%\end{proof}
...
The following is the first version of Hensel's lemma for $\pi$-adics. \fixme{COMPARE TO ELEMENTARY STATEMENT IN TERMS OF MODS.}
...
\end{lem}
\begin{proof}
Since $\ol g$ and $\ol h$ are relatively prime in $k[X]=(A/\mm)[X]$, $(\ol g, \ol h)=A[X]/\mm A[X]$ and $(g,h)+\mm A[X]=A[X]$. Since $A[X]/\mm A[X]$ is finitely generated (on account of $g,h$ being monic), %see ``$\implies$" in Lemma~\ref{criterion-for-integrality}
...
Neukirch, p. 135.
\end{proof}
\fixme{Fill in: local fields vastly simplify things because...}
%
...
\]
the last equality following since $\ol{g}$ is the minimal polynomial of $\ol{\al}$. % and has no repeated roots.
But $[L:K]\ge[l:k]$, so equality holds and $\mfp$ (the prime ideal of $\sO_K$) is unramified by~(\ref{doesnt-ramify}). %Note $L/K$ is separable because $g$ has no repeated roots.
...
\]
\end{thm}
We call $L_u$ the \textbf{maximal unramified extension} of $K$ contained in $L$. \fixme{This is useful...} \fixme{Defining this when $L/K$ is infinite}
\begin{proof}
...
%\end{lem}
%\begin{proof}
%Let $\mfp=(\pi)$ and $\mP=(\Pi)$ be the prime ideals of $\sO_K$ and $\sO_L$, respectively. Then $\pi=c\Pi^e$ for some $c\in \sO_L^{\times}$. Letting $f$ be the residue degree,
%\[
